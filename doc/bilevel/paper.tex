...
The \code{pyomo.bilevel} package uses the \code{SubModel} component to declare a block that defines a lower-level problem. 
...
\end{qlisting}
This Pyomo model defines four variables, \code{x}, \code{v}, \code{sub.y} and \code{sub.w}.
...
While this syntax mimics the structure of (\ref{eqn:bilevel-compact}), it adds additional abstractions that make it difficult to interpret:
\begin{itemize}
